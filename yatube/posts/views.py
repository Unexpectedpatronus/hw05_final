...
def pagination(request, object_list, per_page=POSTS_PER_PAGE):
...
def index(request):
    ...
    page_obj = pagination(request, object_list=post_list)
    context = {
        'page_obj': page_obj,
    }
...
def group_posts(request, slug):
    ...
    page_obj = pagination(request, object_list=posts)
    context = {
        'group': group,
        'page_obj': page_obj,
    }
...
def profile(request, username):
    ...
    following = Follow.objects.filter(
        user=request.user.is_authenticated, author=profile).exists()
    context = {
        'profile': profile,
        'page_obj': page_obj,
        'following': following
